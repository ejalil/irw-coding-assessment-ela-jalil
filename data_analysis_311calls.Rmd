...
clean_2024 %>% 
  ...
  count() %>% 
  arrange(desc(n))
#There were five streets in 2024 that were called in complaints more than 100 times, with 130 M Street NE calling in a complaint 197 times. 
...
```{r}
data_wrapper <- clean_2024 %>%
  group_by(zipcode) %>% 
  count() 
 write.csv(data_wrapper,"~/Downloads/data_wrapper.csv", row.names = FALSE)
```
