...
I now want to shift focus to the street that got the most 311 calls overall. I did this analysis on the most common street, but when I performed further research I realized that it was the office building of the Office of Unified Communication - which is in charge of managing 311 calls. This made sense to why the majority of service calls were centered around DC Govt Info.
...
I want to see if the most common services are found throughout the years - I will do a three year analysis.
...
clean_2023 %>% 
  ...
  count() %>% 
  arrange(desc(n))
#Bulk collection (67,865 calls), parking enforcement (57,413 calls), scheduled yard waste (27,647 calls), rodent inspection and treatment (17,831 calls) and illegal dumping (16,842 calls).

clean_2022 %>% 
  group_by(servicecodedescription) %>% 
  count() %>% 
  arrange(desc(n))
#Bulk Collection (67,924 calls), parking enforcement (53,549 calls), trash collection-missed (18,062 calls), scheduled yard waste (18,017 calls) and illegal dumping (13,978 calls). 

clean_2021 %>% 
  group_by(servicecodedescription) %>% 
  count() %>% 
  arrange(desc(n))
#Bulk Collection (69,714 calls), parking enforcement (41,432 calls), trash collection missed (16, 161 calls), abandoned vehicle - on public property (14,808 calls) and illegal dumping (14, 084 calls). 

#It looks like the same calls are the most frequent to 311. 
#Question: Is there a better way to call for bulk collection?
```
...
Making a line chart to show how certain zip codes are called more frequently than others.
...
zip_long %>%
  ...
  labs(
    ...
    color = "ZIP Code",
    title = "Five Zip Codes With Highest 311 ",
    subtitle = "Each line represents a zip code, with lines ending where the zip code is no longer in the top five.",
    caption = "Source: DC Open Data (311 Service Requests 2015-2024)"
  ) +
  theme_minimal() +
  theme(
